...
class TemplateClimate(TemplateEntity, ClimateEntity, RestoreEntity):
    ...
    def __init__(self, hass: HomeAssistant, config: ConfigType, unique_id: str | None):
        ...
        self._swing_horizontal_mode_template = config.get(
            CONF_SWING_HORIZONTAL_MODE_TEMPLATE
        )
        ...
        if set_swing_horizontal_mode_action := config.get(
    ...
    async def async_set_swing_horizontal_mode(self, swing_mode: str) -> None:
        ...
        if self._set_swing_horizontal_mode_script:
            await self.async_run_script(
                ...
                run_variables={ATTR_SWING_HORIZONTAL_MODE: swing_mode},
                context=self._context,
            )
